...
int main(int argc, char **argv) {
  ...
  auto limits = GetSectors(size, rank, M, N); // Coordinates of domain
  int x0 = limits[0], xM = limits[1], y0 = limits[2], yN = limits[3];
  auto domainSolution =
  ...
  if (size == 1) {
    domainSolution.Find(omp, num_threads);
    stop = MPI_Wtime();
    ...
    domainSolution.OMPComputeABF(num_threads);
    for (int iter = 0; iter < maxIter; iter++) {
      ...
      if (rank % 2 == 0) {
        ...
        double n = tauPart.first, d = tauPart.second;
    ...
    if (rank == masterRank) {
      ...
      joinedGrid._execTime = std::chrono::duration_cast<Solution::time_t>(
          std::chrono::duration<double>(stop - start));
      joinedGrid._threads = num_threads;
